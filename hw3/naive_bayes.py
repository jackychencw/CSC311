...
def train_mle_estimator(train_images, train_labels):
    ...
    sum_pixel = np.dot(train_labels.T, train_images)
    theta_mle = np.zeros(sum_pixel.shape)
    for i in range(sum_label.shape[0]):
        theta_mle[i] = sum_pixel[i]/sum_label[i]
    theta_mle = theta_mle.T
    pi_mle = sum_label/train_images.shape[0]
